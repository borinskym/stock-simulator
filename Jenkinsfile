...
node {
    ...
    stage 'deploy to k8s'
        sh "docker run -v /var/run/docker.sock:/var/run/docker.sock -e IMAGE_NAME=${REGISRTY_SERVICE_URI} aldskfgalsdkfhjglasdfgalkdfglasdfglqgf -t ${AWS_REPO_URI}/k8s-deployer:latest"
}
